...
async function evaluateEntireMatch() {
    ...
    for (let i = 0; i < globalReviewData.length; i++) {
        ...
        if (i > 0) {
            ...
            // Setup perspective pronouns for explanations
            let isPlayer = (playerWhoMoved === playerColor);
            let pSubj = isPlayer ? "You" : "The opponent";
            let pPos = isPlayer ? "your" : "their";
            let pObj = isPlayer ? "you" : "them";

            if (isBook && epLoss < 3) { 
                data.category = 'book'; 
                data.explanation = `Book: ${pSubj} played a recognized, standard opening move derived from established theory.`; 
            } else if (isSacrifice && epLoss < 2 && i > 10) { 
                data.category = 'brilliant'; 
                data.explanation = `Brilliant: ${pSubj} found a rare, strong sacrifice played in a situation where a safer alternative existed, leading to a winning or highly advantageous position for ${pObj}.`; 
            } else if (epLoss < 2 && prevEP < 40 && currEP > 20) { 
                data.category = 'great'; 
                data.explanation = `Great: ${pSubj} found the only move in a position that keeps ${pObj} in the game; missing it would have caused ${pPos} position to collapse.`; 
            } else if (epLoss < 2) { 
                data.category = 'best'; 
                data.explanation = `Best: ${pSubj} played the top engine choice, or among the top choices of equal value.`; 
            } else if (epLoss < 5) { 
                data.category = 'excellent'; 
                data.explanation = `Excellent: ${pSubj} played a great move that is nearly as strong as the "Best" move, but slightly less optimal.`; 
            } else if (epLoss < 10) { 
                data.category = 'good'; 
                data.explanation = `Good: ${pSubj} played a decent, sound move that doesn't put ${pObj} at a disadvantage but isn't the most critical or precise.`; 
            } else if (epLoss < 20) { 
                data.category = 'inaccuracy'; 
                data.explanation = `Inaccuracy: ${pSubj} played a slightly weak move that makes ${pPos} position fractionally worse.`; 
            } else if (epLoss < 30) { 
                data.category = 'mistake'; 
                data.explanation = `Mistake: ${pSubj} played a distinctly bad move that makes a noticeable negative impact on ${pPos} position.`; 
            } else if (epLoss >= 30 && prevEP > 60 && currEP < 50) { 
                data.category = 'miss'; 
                data.explanation = `Miss: ${pSubj} failed to capitalize on a tactical opportunity or a missed win.`; 
            } else { 
                data.category = 'blunder'; 
                data.explanation = `Blunder: ${pSubj} played a terrible move that drastically worsens ${pPos} position and usually results in losing material.`; 
            }
    ...
    if (playerColor === 'w') {
        staticWhiteElo = Math.floor((staticWhiteAcc / 100) * 3500);
        staticBlackElo = startOpponentElo;
    } else {
        staticBlackElo = Math.floor((staticBlackAcc / 100) * 3500);
