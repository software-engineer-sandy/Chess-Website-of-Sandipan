...
async function getAbsoluteEval(fen) {
    // Explicitly enforce perfectly balanced 0 cp evaluation at the initial starting position
...
async function evaluateEntireMatch() {
    ...
    for (let i = 0; i < globalReviewData.length; i++) {
        $('#review-loading p').html(`Analyzing match...<br><b>${Math.round((i / globalReviewData.length) * 100)}%</b>`);
    ...
    // Assign Opponent ELO from Start Game matching input. Compute Player ELO natively.
    if (playerColor === 'w') {
...
function applyReviewMoveUI() {
    ...
    // Append end-of-game text if user is on the final frame
...
$(document).ready(() => {
    ...
    // Fire off Background evaluation wrapper loop
    evaluateEntireMatch();
});
