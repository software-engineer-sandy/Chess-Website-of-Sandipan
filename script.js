...
function getSquareCoordinates(square) {
    ...
    const squareSize = 500 / 8; // Preserved at 500 because the SVG viewBox scales natively 
...
function drawArrow(move, color, offsetIndex = 0) {
    ...
    // Calculate a parallel shift distance to separate overlapping arrow tails 
    let shift = 0;
    if (offsetIndex > 0) {
        shift = (offsetIndex % 2 === 0 ? -1 : 1) * Math.ceil(offsetIndex / 2) * 8;
    }
    ...
    line.setAttribute('stroke-width', '4'); // Reduced stroke width
...
function setupSVGDef() {
    ...
    ['green', 'red'].forEach(color => {
        ...
        polygon.setAttribute('points', '0 0, 6 2.1, 0 4.2'); // Scaled down polygon points
...
async function updateAssistance() {
    ...
    // Limit to 2 PVs (Principal Variations)
    ...
    const extractMoves = (output) => {
        ...
        // Max 2 recommended moves 
        return [...new Set(moves)].slice(0, 2); 
    };

    // Dictionary to track tails mapped to their starting square
    ...
    extractMoves(playerOutput).forEach(move => {
        ...
        tailOverlapCounts[fromSq] = count + 1; // Increment offset index for this square
    });
...
// Ensures chessboard natively resizes with the window 
